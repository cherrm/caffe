...
void MaxMarginLossLayer<Dtype>::Forward_gpu(
...
__global__ void CLLBackward(const int count, const int channels,
		const Dtype margin, Dtype *B, const Dtype gpuC, const Dtype alpha,
	...
	CUDA_KERNEL_LOOP(i, count) {
		...
		if (yij * (*B - dist_sq[n]) <= margin) {
			Dtype scalar = alpha * (2.0) * yij * diff[i];
			*B += alpha * yij * gpuC;
			bottom_diff[i] = scalar;
		}
		else {
			bottom_diff[i] = 0;
		}
...
template <typename Dtype>
void MaxMarginLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(MaxMarginLossLayer);
}
