...
std::string ReadFileToDatum(const string& filename, const int label) {
  std::streampos size;
...
int main(int argc, char** argv) {
  ...
  for (int itemid = 0; itemid < num_triples; ++itemid) {
    ...
    if (cv_img[i].data && cv_img[1].data && cv_img[2].data) {
      if (encode_type.size()) {
        if ((cv_img[0].channels() == 3 && cv_img[1].channels() == 3 && cv_img[2].channels() == 3) == is_color 
          ...
          cv::imencode("."+encoding, cv_img[0], tmp);
          buffer.insert(std::end(buffer), std::begin(tmp), std::end(tmp));
          cv::imencode("."+encoding, cv_img[1], tmp);
          buffer.insert(std::end(buffer), std::begin(tmp), std::end(tmp));
          cv::imencode("."+encoding, cv_img[2], tmp);
          buffer.insert(std::end(buffer), std::begin(tmp), std::end(tmp));
